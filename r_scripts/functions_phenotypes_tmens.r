...
set_tmens_names <- function(x){
  names1 <- str_replace_all(x,c(a1 = "niche1",
                                a2 = "niche2",
                                a3 = "niche3",
                                a4 = "niche4",
                                a5 = "niche5",
...
plot_3D_ternary_cell_markers <- function (cellType, marker){
  MarkersCellsTMENS.ct <- MarkersCellsTMENS%>%
    ...
    pivot_wider(id_cols = c("site_id","patient_id","arch1","arch2","arch3","arch4", "arch5", "arch6", "achr7"),
                names_from = "marker",
                values_from = "value")
  X <- rbind(c(1, 0, 0),
             c(0, 1, 0),
  ...
  Coords.3D <- bary2cart(X,as.matrix(MarkersCellsTMENS.ct [,c("arch1","arch2","arch3","arch4", "arch5", "arch6", "achr7")]))
...
correlations_tmens_CM <- function(MarkersCellsTMENs,cellTypes, markers, qThresh=1/100, corThresh=0.3){
  corMatrix.pval <-
    matrix(nrow = length(cellTypes)*length(markers), ncol=4+6,
           dimnames = 
             list(c(),
                  c('a1','a2','a3',"a4", "a5", "a1a2","a1a3","a1a4","a2a3","a2a4","a3a4", "a1a5", "a2a5", "a3a5", "a4a5")))#, ,"a1a2a3","a1a2a4","a1a3a4","a2a3a4","a1a2a3a4")))
  ...
  for(ct in cellTypes){
    ...
    for(m in markers){
      ...
      corMatrix.pval[id,"a5"] <- cor.test(X%>%pull(value),A1%>%pull(arch5),method="spearman",exact=FALSE)$p.value
      corMatrix.pval[id,"a6"] <- cor.test(X%>%pull(value),A1%>%pull(arch6),method="spearman",exact=FALSE)$p.value
      corMatrix.pval[id,"a7"] <- cor.test(X%>%pull(value),A1%>%pull(arch7),method="spearman",exact=FALSE)$p.value
      ...
      corMatrix[id,"a5"] <- cor.test(X%>%pull(value),A1%>%pull(arch5),method="spearman",exact=FALSE)$estimate
      corMatrix[id,"a6"] <- cor.test(X%>%pull(value),A1%>%pull(arch6),method="spearman",exact=FALSE)$estimate
      corMatrix[id,"a7"] <- cor.test(X%>%pull(value),A1%>%pull(arch7),method="spearman",exact=FALSE)$estimate
      ...
      corMatrix.pval[id,"a1a5"] <- cor.test(X%>%pull(value),A2%>%pull(a1a5),method="spearman",exact=FALSE)$p.value
      corMatrix.pval[id,"a2a5"] <- cor.test(X%>%pull(value),A2%>%pull(a2a5),method="spearman",exact=FALSE)$p.value
      corMatrix.pval[id,"a3a5"] <- cor.test(X%>%pull(value),A2%>%pull(a3a5),method="spearman",exact=FALSE)$p.value
      corMatrix.pval[id,"a4a5"] <- cor.test(X%>%pull(value),A2%>%pull(a4a5),method="spearman",exact=FALSE)$p.value
      ...
      corMatrix[id,"a1a5"] <- cor.test(X%>%pull(value),A2%>%pull(a1a5),method="spearman",exact=FALSE)$estimate
      corMatrix[id,"a2a5"] <- cor.test(X%>%pull(value),A2%>%pull(a2a5),method="spearman",exact=FALSE)$estimate
      corMatrix[id,"a3a5"] <- cor.test(X%>%pull(value),A2%>%pull(a3a5),method="spearman",exact=FALSE)$estimate
      corMatrix[id,"a4a5"] <- cor.test(X%>%pull(value),A2%>%pull(a4a5),method="spearman",exact=FALSE)$estimate
...
correlation_niches_CM <- function(markersCells.niches,Markers,corrMeth="spearman",coreIntf2=(all_of(coreIntf2)),qThresh=1/100,corThresh=0.3,nbNiches){
  rareCells<- markersCells.niches%>%group_by(cell_type)%>%summarise(total=n())%>%filter(total<=3*length(Markers))%>%pull(cell_type)
  ...
  small_sample <- markersCells.niches %>% 
    filter(!cell_type %in% rareCells) %>% # Remove rare cell types
    filter(marker %in% Markers) %>%       # Filter markers
    sample_frac(0.1)                      # Take a 10% random sample of rows
  
  CM.gps <- small_sample %>%
    pivot_longer(cols=all_of(coreIntf2), names_to = "niche", values_to = "weight") %>%
    mutate(id = paste(cell_type, marker, niche, sep = ";")) %>%
    mutate(threshNiche = ifelse(niche %in% paste0("a", as.vector(seq(1, nbNiches, 1))), 1/2, 1/8)) %>%
    group_by(cell_type, marker, niche) %>%
  ...
  if(ncol(corQval)!=length(coreIntf2)){
    corQval[,coreIntf2[!(coreIntf2 %in% colnames(corQval))]]<-0
  }
  print("ok")
  ...
  #Recover all possible niches and interfaces with the cell phenotypes
  if(ncol(corMatrix2 )!=length(coreIntf2)){
    corMatrix2 [,coreIntf2[!(coreIntf2 %in% colnames(corMatrix2 ))]]<-0
  }
...
get_CT_enriched_all_archs <- function(archsSitesCellAb,archNames,thresh=0.99){
  ...
  #archetypes.ct <- list() 
  #names(archetypes.ct) <- archs
  archNames <- NichesNames
  replacement_values <- c("a1" = "niche1", 
                          "a2" = "niche2", 
                          "a3" = "niche3", 
                          "a4" = "niche4")
  ...
  archetypes_cts.all$niche <- str_replace_all(archetypes_cts.all$niche, setNames(replacement_values, archNames))
    #str_replace_all(archetypes_cts.all$niche,archNames)#set_tmens_names(archetypes_cts.all$niche)
  return(archetypes_cts.all)#(archetypes.ct)
}
...
TableNichesPhenotypes <- function(CM,NichesCT,Niches.names,nichesCA.sorted,pathFigs){
  ...
  tabTMENs2 <- as_tibble(CM,rownames=NA)%>%
    ...
    pivot_longer(cols=any_of(coreIntf2),names_to = "niche",values_to="corr_val")#%>%
  ...
  tabCellPhen <- tabTMENs2%>%
    separate(col=niche, into = c("type1","type2"),sep="x", fill="right")%>%
  ...
  tabCellPhen3 <- tabCellPhen2%>%
    ...
    distinct(cell_phenotype, .keep_all = TRUE)%>%
    ungroup()%>%
    data.frame()%>%
    arrange(niche)
  
  tabCellPhen3_expanded <- tabCellPhen3 %>%
    separate_rows(cell_phenotype, sep = "\n") %>%
    mutate(row_id = row_number()) # Add a row identifier
  
  n_rows <- nrow(tabCellPhen3_expanded)
  n_cols <- ncol(tabCellPhen3) # Assuming tabCellPhen3 has 2 columns: niche and cell_phenotype
  
  g <- gtable(widths = unit(rep(1, n_cols), "null"), # Equal column widths
              heights = unit(rep(1, n_rows), "null")) # Equal row heights
  
  for (i in seq_len(n_rows)) {
    for (j in seq_len(n_cols)) {
      cell_text <- as.character(tabCellPhen3_expanded[i, j])
      g <- gtable_add_grob(
        g,
        grobs = textGrob(cell_text, x = 0.5, y = 0.5, just = "center"),
        t = i, l = j, b = i, r = j # Set the cell position
      )
    }
  }
  
  write.csv(tabCellPhen3_expanded, "./tabNichePhen.csv")
}
